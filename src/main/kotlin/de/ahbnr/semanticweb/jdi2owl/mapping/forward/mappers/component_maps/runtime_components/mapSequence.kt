...
fun mapSequence(context: SequenceContext) = with(context) {
    ...
    // The following code closes the range set of hasElement for this container object
    //
    ...
    for ((idx, elementValue) in elements.withIndex()) {
        ...
        with (IRIs) {
            tripleCollector.dsl {
                elementInstanceIRI {
                    ...
                    rdf.type of typedSequenceElementIRI
                    java.hasIndex of NodeFactory.createLiteralByValue(idx, XSDDatatype.XSDint)
                }
            }
        }
        ...
        if (idx < elements.size - 1) {
            ...
            if (idx == 0) {
                // encode that there is no predecessor for first element
                tripleCollector.addStatement(
                    elementInstanceIRI,
                    IRIs.rdf.type,
                    tripleCollector.addConstruct(
                        TripleCollector.BlankNodeConstruct.OWLObjectCardinalityRestriction(
                            onProperty = tripleCollector.addConstruct(
                                TripleCollector.BlankNodeConstruct.OWLInverseProperty(IRIs.java.hasSuccessor)
                            ),
                            onClassUri = IRIs.java.SequenceElement,
        ...
        val valueNode = mapValue(elementValue, this) ?: continue
